...
const findRouteById = (routeId) => {
  const normalizedId = normalizeRouteUidValue(routeId);
  if (!normalizedId) {
    return null;
  }
...
function updateRouteScoreEntry(routeId, userId, details) {
  const normalizedRouteId = normalizeRouteUidValue(routeId);
  const normalizedUserId = typeof userId === 'string' ? userId.trim() : '';
  ...
  if (entries.length) {
    entries.forEach(({ betatipId, userId, username, text, isCurrent, upvoteCount, hasUpvoted }) => {
      ...
      if (hasUpvoted) {
        ...
        icon.title = 'You upvoted this beta tip';
      }
      ...
      if (Array.isArray(ariaLines)) {
        ...
        const youUpvoted = hasUpvoted ? ' You upvoted this beta tip.' : '';
        ariaLines.push(
          `Beta from ${username}${isCurrent ? ' (you)' : ''}: ${ariaText} (${upvoteSummary}).${youUpvoted}`,
        );
      }
    });
  } else {
...
async function loadRoutes() {
  try {
    ...
    scoresSnapshot.forEach((docSnap) => {
      ...
      const routeUid = normalizeRouteUidValue(routeRef?.id);
      const userUid = normalizeRouteUidValue(docSnap.id);
...
function getRouteDisplayColor(route) {
  ...
  return isColorblindModeEnabled() ? mapHoldColorToColorblindPalette(holdColor) : holdColor;
}
